...
const HomeSlider = () => {
  ...
  return (
    <div className="relative w-full h-[60vh] overflow-hidden">
      ...
      {slides.map((slide, index) => (
        ...
        >
          <div className="flex flex-col items-center justify-center h-full text-white px-4">
            ...
            <a 
              to="/about-us" 
              className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-8 rounded-full transition-colors duration-300 mt-4 cursor-pointer"
            >
              Read More
            </a>
          </div>
      ...
      {/* Navigation Arrows */}
      <button
        className="absolute top-1/2 left-4 z-20 bg-black bg-opacity-50 text-white p-3 rounded-full transform -translate-y-1/2 cursor-pointer"
        onClick={prevSlide}
      >
        <FaArrowLeft />
      </button>
      ...
      <button
        className="absolute top-1/2 right-4 z-20 bg-black bg-opacity-50 text-white p-3 rounded-full transform -translate-y-1/2 cursor-pointer"
        onClick={nextSlide}
      >
        <FaArrowRight />
      </button>
